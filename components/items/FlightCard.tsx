...
export function FlightCard({ item, onDelete, onEdit, readonly }: FlightCardProps) {
        ...
        <div className="px-5 pb-4 pt-1 flex items-center justify-between">
          <span className="text-[#6B8FA8] text-xs">
            {item.price && item.price > 0
              ? <>Totale <span className="text-aqua font-medium">€{item.price.toFixed(2)}</span></>
              : null}
          </span>
          <div className="flex gap-4">
            <button onClick={onEdit}
              className="text-sand/30 hover:text-sand text-xs transition-colors">
              Modifica
            </button>
